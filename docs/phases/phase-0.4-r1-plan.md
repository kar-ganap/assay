...
### Verdicts — REVISED 2026-08-06 after the three-reviewer pass

- **G3 — partial.** Magnitudes broadly reproduce, ordering does not.
- **G4 / R1-P — UNRESOLVED.** *(The first version of this section said "FALSIFIED AS WRITTEN". That
  was wrong and is corrected here.)* R1-P claims no word with a higher base rate saturates later;
  the observed order **is** the base-rate order, ρ = +1, zero inversions, with the non-significant U
  in R1-P's own direction. The committed scorer prints `UNRESOLVED`. Failing to reject is not
  falsifying — and reporting falsification on an indeterminate result is the same missing-cell error
  as §3.4, one cell the other way.
  **What R1 does establish:** the shift `midnight − ocean` is +4.99 steps, 95% CI [−7.84, +9.85],
  which **excludes a Prime-sized ordering effect (−26)** and does not exclude zero.
- **R1-P′** registered as new-and-untested: base rate predicts onset across order-of-magnitude gaps
  and not small ones. **Not reportable as supported by R1** — and thinner than it looks: both its
  contrasts sit at their design floors and Holm puts `forgotten < midnight` at 0.0571.

### The consequence that DOES fire — and it is not the one this table predicted

Branch 1 warned that if base rate fails to order onset, *"L1's admission band needs redesign."* That
trigger is about **ranking** and does not fire on an unresolved result.

**A different and larger one fires instead.** Two of three words — `ocean` (0.0135) and `midnight`
(0.0059) — sit **below** the band's lower bound of `1/64 = 0.015625`, and both saturated inside 100
steps where §4 says "unreachable in 200". A literal k=64 screen would have missed them 42% and 68%
of the time. `1/64` is the *resolution floor* of a 64-sample screen, not a reachability threshold,
so **k is the defect and the band needs redesigning before any Run grid.** Recorded in
`pre-registration.md` §4.

*(An earlier version of this section said "every word inside our band saturated, so the band's job
is supported." True of n = 1 variant, and it buried the disconfirmation.)*
