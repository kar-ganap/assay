...
\large A worked tutorial, with the five things that actually bit us}}
...
0.1. \Cref{sec:surprises} is the part worth the most: five places where the standard account is
either incomplete or wrong at our scale, each one found by a measurement in this phase rather than
taken from the literature.

Every numerical claim in \cref{sec:ablations,sec:surprises} comes from the clean ladder of
2026-08-02 --- ten arms on one training-path fingerprint and one GPU tier, 200 steps each --- and
regenerates from committed artefacts via \texttt{assay.crawl.figures}. Arms carrying a claimed
\emph{difference} were run at three seeds and are reported as mean $\pm$ half the min--max envelope;
rungs 1--3 are $n=1$ by design and say so where they appear.
\end{abstract}
...
\subsection{Two rewards: proxy and true}
\label{sec:proxytrue}

There is a second reward, and the distinction between them is the instrument this entire project is
built around. Every completion is graded \emph{twice}:

\begin{itemize}
  \item $R_{\text{proxy}}(x,y)$ --- what the policy is \textbf{trained on}. It enters the loss. This
        is the grader you can actually run at training time.
  \item $R_{\text{true}}(x,y)$ --- what you actually \textbf{want}. It is computed at every step and
        \textbf{never enters the loss}. Measured only.
\end{itemize}

The quantity of interest is their difference,
\begin{equation}
  \mathrm{gap} \;=\; R_{\text{proxy}} - R_{\text{true}},
  \label{eq:gap}
\end{equation}
which is reward the proxy awards that the truth does not endorse --- Goodhart's law, as a number.

In Phase 0.1, $R_{\text{true}}$ is \emph{always} $R_{\text{binary}}$; only the proxy varies:

\begin{table}[H]
\centering
\caption{The three reward variants. Only the proxy changes, which is what makes gaps comparable
across arms.}
\label{tab:variants}
\small
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Variant} & \textbf{Used by} & \textbf{$R_{\text{proxy}}$} & \textbf{Gap} \\
\midrule
\texttt{binary}   & ladder runs 1--7 & $R_{\text{binary}}$ & $\equiv 0$ by construction \\
\texttt{format}   & ablation B & shape only; the answer is discarded & grows: $0.519 \pm 0.011$ \\
\texttt{tiebreak} & ablation C & $R_{\text{binary}} + 0.001\,|y|$ & exactly $0.001\,|y|$: $0.039 \pm 0.002$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The ladder's gap is identically zero, and that is correct.} Runs 1--7 set proxy $=$ true, so
\cref{eq:gap} vanishes. The ladder is about the \emph{estimator} --- whether the gradient works ---
not about Goodhart. A nonzero gap on run 7 would indicate a bug, not a finding. The gap only becomes
a meaningful quantity once a deliberately different proxy is introduced, which is what ablations B
and C do.

\textbf{Ablation C doubles as a calibration of the instrument.} Its gap is analytically known ---
the tie-breaker pays exactly $0.001$ per token, so the gap \emph{must} equal $0.001\,|y|$. Measured
over the final ten steps of three seeds: gap $= 0.03899$ against $0.001 \times 38.99 = 0.03899$, a
ratio of $1.0000$. When a measurement has a closed form, check it.

\begin{aside}
\textbf{Why the true reward must never be trained on.} This is a contamination argument, not a cost
one. The moment a held-out grader enters the objective it stops measuring generalisation and becomes
a second proxy --- and the gap it existed to reveal closes for the wrong reason. The asymmetry
\emph{is} the instrument.

The real-world version this rehearses: in the \texttt{bisect} environment, the proxy is ``the failing
test passes'' and the truth is ``a held-out suite exercising the same root cause through other code
paths.'' You cannot train on the latter --- if you could specify it perfectly, you would not need RL.

And a caveat that has to travel with every gap ever reported here: \textbf{held-out graders are
graders too.} They can be wrong in both directions, so a gap is only as trustworthy as the true
grader's own false-positive and false-negative rates on a hand-labelled gold set.
\end{aside}
...
\subsection{Notation for averages}
\label{sec:rbar}

Two different averages of reward appear throughout, over the two units just defined. They are
\emph{always} subscripted here, because conflating them is an easy and consequential mistake --- it
is the entire difference between rung 2 and rung 3 of the ladder.

\begin{align}
  \bar{r}_{\text{group}} \;&=\; \frac{1}{G}\sum_{j=1}^{G} r_j
    &&\text{mean over the $G$ rollouts of \emph{one} prompt,} \\
  \bar{r}_{\text{batch}} \;&=\; \frac{1}{N}\sum_{i=1}^{N} r_i
    &&\text{mean over all $N$ rollouts in a gradient step.}
\end{align}

Distinguish both from $p$, which denotes the \emph{true} success probability $\Pr[R = 1]$ --- a
property of the policy and the task, not of any particular sample. We never observe $p$; we observe
$\bar{r}$, which is an unbiased but noisy estimate of it. At $G = 8$, $\bar{r}_{\text{group}}$ takes
only nine possible values, so it is a \emph{very} noisy estimate of that prompt's difficulty. This
matters more than it looks (\cref{sec:varcalc,sec:eweight}).
...
\begin{aside}
\textbf{\Cref{eq:optb} is not the thing anyone actually computes.} Look at what $b^\star$ requires:
$p$, $E_c$ and $E_w$ --- the true success probability and two conditional second moments of the
score. None of the three is observable. So every implementation substitutes the one quantity that is
free, the sample mean reward $\bar{r}$ (\cref{sec:rbar}), which is a plug-in estimate of $p$.

That substitution is exact only when $E_c = E_w$: setting them equal in \cref{eq:optb} collapses it
to $b^\star = p$, estimated by $\bar{r}$. Otherwise using $\bar{r}$ is a heuristic --- a
\emph{reasonable} one, since the correction factor is bounded and the resulting estimator is still
unbiased by \cref{claim:unbiased}, but a heuristic nonetheless. \Cref{sec:eweight} is what our own
measurements say when that assumption fails, and here it fails badly enough to erase the entire
benefit.
\end{aside}
...
$\bar{r}_{\text{batch}} = 0.43$, and three groups of differing difficulty.
...
\begin{equation}
  ...
  \qquad \sigma^2_{\text{group}} = \frac{1}{G}\sum_j \big(r_j - \bar{r}_{\text{group}}\big)^2 .
  \label{eq:znorm}
\end{equation}
...
\subsubsection*{What ``degenerate'' and ``hackable'' mean}

Two terms that get used loosely, and which are worth separating precisely because they are properties
of \emph{different things}.

A grader is \textbf{degenerate} when its score does not depend on whether the task was performed. For
$R_{\text{format}}$ this is not a figure of speech --- it is one line of the implementation:
\begin{quote}\ttfamily\small
def grade\_format\_only(completion, expected):\\
\hspace*{1em}del expected \ \ \# deliberately unused --- that *is* the pathology\\
\hspace*{1em}\ldots\ return reward 1.0 if completion matches <answer>N</answer> else 0.0
\end{quote}
The correct answer is \emph{discarded before grading}. Anything shaped like
\texttt{<answer>N</answer>} pays $1.0$, so the constant string \texttt{<answer>0</answer>} scores full
marks on every prompt in the distribution. \textbf{The reward-maximising policy is a constant function
that never reads its input.}

``Degenerate'' is meant in the mathematical sense --- a structure has collapsed. A degenerate conic is
a pair of lines rather than an ellipse; a degenerate reward is one whose level sets no longer separate
correct from incorrect.

A grader is \textbf{hackable} when a degenerate maximum both exists \emph{and} is cheaply reachable
from where the policy starts. The distinction matters:

\begin{itemize}
  \item \textbf{Degeneracy} is a property of the grader alone --- inspectable statically, without
        running anything.
  \item \textbf{Hackability} is a property of the (grader, policy, budget) triple. A degenerate
        grader whose exploit is astronomically unlikely under the base policy will not be hacked in
        200 steps, because RL at this scale \emph{amplifies what is already in the policy's support}
        rather than discovering new behaviour.
\end{itemize}

That second point is why this project screens $p_{\text{hack}}@64$ on the \emph{base} policy before
admitting any grader variant, and why ``nothing hacks at 1.7B in 200 steps'' is recorded as its single
largest risk. A diagnostic that flags degeneracy is measuring the grader; whether that degeneracy
\emph{bites} depends on the model and the budget.

\textbf{``Deliberately'' hackable} means $R_{\text{format}}$ was built this way as a \emph{positive
control}. If one claims a diagnostic detects grader degeneracy, at least one environment must have a
known answer --- otherwise a null result cannot be interpreted, since ``the diagnostic failed'' and
``there was nothing to find'' look identical.

\subsubsection*{What it looks like when the grader is hacked}

\begin{table}[H]
\centering
\caption{Ablation B's control arm over training, mean of three seeds. The proxy is learned in ten
steps; the skill never moves at all.}
\label{tab:bgap}
\small
\begin{tabular}{@{}rrrrr@{}}
\toprule
\textbf{step} & \textbf{proxy} & \textbf{true} & \textbf{gap} & \textbf{distinct completions} \\
\midrule
0   & 0.143 & 0.471 & $-0.328$ & 101 \\
5   & 0.380 & 0.367 & $+0.013$ & 95 \\
10  & 0.917 & 0.477 & $+0.440$ & 61 \\
20  & 0.984 & 0.422 & $\mathbf{+0.562}$ & 66 \\
199 & 0.990 & 0.523 & $+0.466$ & 71 \\
\bottomrule
\end{tabular}
\end{table}

Three things to read off \cref{tab:bgap}.

\textbf{The gap starts negative.} The base model already solves $47.1\%$ of the additions but emits
the required tag only $14.3\%$ of the time, so initially the proxy \emph{understates} true ability.
Reward hacking is not visible at step 0; it has to be trained in.

\textbf{The proxy saturates in ten steps and the skill does not move.} Averaged over the first and
last ten steps of three seeds, the proxy goes $0.397 \to 0.993$ while true reward goes $0.453 \to
0.474$ --- a change of $+0.021$ whose sign flips across seeds ($+0.032$, $-0.004$, $+0.034$), which
is to say: nothing. Two hundred steps bought sixty points of proxy and zero points of arithmetic.

\begin{aside}
\textbf{A seed-0 artefact, corrected.} At $n=1$ this arm's true reward appeared to \emph{fall}
($0.516 \to 0.445$), and an earlier draft of this document said optimising the format ``actively
damaged arithmetic''. At $n=3$ it is flat. The single-seed dip was noise, and the flat version is the
stronger claim anyway --- a degenerate grader does not need to damage the skill to be worth
worrying about; it only needs to stop teaching it.
\end{aside}

\textbf{The completions make it unambiguous.} On \texttt{875 + 812} (answer: $1687$), the policy at
step 150 emitted \texttt{<answer>1677</answer>}, \texttt{<answer>1707</answer>},
\texttt{<answer>1677</answer>}, \texttt{<answer>1756</answer>} --- perfectly formatted, every one
wrong. Two hundred steps of gradient descent spent learning punctuation.

\begin{aside}
\textbf{This is why the true reward must never be trained on.} $R_{\text{binary}}$ is computed at
every step of ablation B and thrown away --- it never touches the loss. The moment a held-out grader
enters the objective it stops measuring generalisation and becomes a second proxy, and the gap it was
supposed to reveal closes for the wrong reason. The asymmetry is the instrument.
\end{aside}

\begin{table}[H]
\centering
\caption{Ablation B, three seeds, mean $\pm$ half the min--max envelope. Proxy is what is optimised;
true is $R_{\text{binary}}$, measured and never trained on. The gap is the Goodhart quantity.}
\label{tab:abB}
\small
...
\midrule
control ($\beta = 0.04$) & $0.993 \pm 0.002$ & $0.474 \pm 0.010$ & $\mathbf{0.519 \pm 0.011}$ & $2.87$ & $0.544$ \\
ablation B ($\beta = 0$)  & $0.999 \pm 0.001$ & $0.517 \pm 0.015$ & $\mathbf{0.482 \pm 0.014}$ & $3.04$ & $0.000$ \\
\bottomrule
\end{tabular}
\end{table}

Two things to take from \cref{tab:abB}. First, the degenerate grader is \emph{comprehensively}
hacked: the policy scores $0.993$ on the proxy while true performance sits at $0.474$ --- it learned
the format and not the task. That $+0.519$ gap is the largest in the whole ladder.

Second, and this is the reverse of what the leash is for: \textbf{removing the KL penalty made the
gap \emph{smaller}}. Paired by seed, the difference is $+0.037$ with the same sign on all three
($+0.018$, $+0.036$, $+0.058$), and the leashed arm also ends with lower true reward on every seed
($0.474$ vs $0.517$) --- below the base model's own $\approx 0.47$. At $\beta = 0.04$, carrying
$54\%$ of the loss, the leash is not restraining anything; it is costing true performance and
slightly widening the gap.

Part of the mechanism runs through ablation D: the degenerate reward drives the dead-group fraction
to $0.895$, so there is very little policy gradient left for a leash to restrain --- a leash is
irrelevant when the horse has stopped. That explains why the effect is \emph{small}. It does not
explain why it is \emph{backwards}, and this document does not have that answer. The obvious probe
is to sweep $\beta$: if the leash genuinely hurts here, the gap should grow with $\beta$.

\begin{aside}
\textbf{This is the finding that most needed three seeds.} At $n=1$ the two arms read $0.507$ and
$0.489$, and an earlier draft reported ``removing the leash changes the gap by $-0.018$, i.e.\
nothing''. That single seed happened to be the smallest of the three differences. The null was an
artefact of $n=1$, and the effect it hid points the opposite way to the intervention's purpose.
\end{aside}
...
\begin{table}[H]
\centering
\caption{Ablation C against its parent, three seeds each. Goodhart emerging from the optimiser's own
arithmetic.}
\label{tab:abC}
\small
\begin{tabular}{@{}lrrrr@{}}
...
\midrule
run 7 (binary)        & $0.571 \to 0.923$ & $0.000$ & $0.472 \pm 0.016$ & $18.6 \pm 1.4$ \\
ablation C (tiebreak) & $0.567 \to 0.716$ & $0.039 \pm 0.002$ & $\mathbf{0.009 \pm 0.002}$ & $\mathbf{34.7 \pm 0.6}$ \\
\bottomrule
\end{tabular}
\end{table}

All four predicted signatures fire, and every one of them clears its seed band: dead groups collapse
$0.472 \to 0.009$, completions nearly double ($18.6 \to 34.7$ tokens), the true-reward gain falls
from $+0.352$ to $+0.149$, and a real proxy--true gap of $0.039 \pm 0.002$ opens on a task where run
7's gap is identically zero by construction.
...
Measured, three seeds: with every group forced unanimous, \texttt{frac\_degenerate\_groups} $= 1.000$
and \texttt{grad\_norm} $= 0.0000$ on all $200$ steps of all three runs --- exactly, with a seed band
of zero. True reward flat at $0.471 \to 0.477$, and tokens generated at the parent's rate.
\textbf{Roughly forty minutes of GPU per seed for exactly zero gradient.}
...
% ══════════════════════════════════════════════════════════════════
\section{Five things the standard account gets wrong at this scale}
\label{sec:surprises}
...
\begin{itemize}
  ...
  \item \texttt{global}: $\Adv_i = r_i - \bar{r}_{\text{batch}}$. Since $\bar{r}_{\text{batch}}$ is an average over 128 rollouts and
...
\begin{table}[H]
\centering
\caption{Dead-group fraction by baseline, averaged over 200 steps. The analytical prediction,
exactly. Rungs 1--3 are $n=1$: they were excluded from the seed pass because these numbers are a
consequence of the advantage arithmetic rather than an empirical effect, so a seed band would measure
noise around a derived value. Ablation D is $n=3$ with a band of exactly zero.}
\label{tab:dead}
\small
\begin{tabular}{@{}llr@{}}
\toprule
...
$0.244$ over the first ten steps to $0.606$ over the last ten ($n=1$), because the policy is solving
more prompts unanimously. Full GRPO is worse still --- $0.260 \pm 0.044 \to 0.715 \pm 0.028$ across
three seeds. The better the policy gets, the larger the share of each batch that produces no gradient
at all: a built-in brake that tightens exactly as the task is being learned. \Cref{sec:underpowered}
follows that brake into the gradient itself.
...
Separately, removing length normalisation improved the noise-to-signal ratio of \emph{every} arm in
the probe ($0.45$--$0.94 \to 0.30$--$0.33$). Its effect on training is more selective than an earlier
$n=1$ draft of this document claimed. On ablation C --- the arm being actively Goodharted --- final
true reward rises $0.716 \to 0.849$, a gain of $+0.134$ with the same sign on all three seeds
($+0.116$, $+0.159$, $+0.126$). On run 7, with a clean binary reward, the gain is $+0.017$ and the
\emph{sign flips} on one seed ($+0.020$, $-0.004$, $+0.034$): no reliable effect. So the honest
version is that removing length normalisation helps where the reward is degenerate and does nothing
detectable where it is not --- support for Dr.\ GRPO's position, but narrower than ``it helps''.
\end{aside}
...
\begin{table}[H]
\centering
\caption{Observed ratio against the point prediction, base policy, $N=160$ batches, paired
bootstrap. An earlier draft reported the $N=40$ run; quadrupling the batches roughly halved every
interval and did not move the conclusion.}
\label{tab:nsr}
\small
\begin{tabular}{@{}rrrr@{}}
\toprule
...
\midrule
0 & 0.931 & $[0.787,\ 1.091]$ & 1.868 \\
1 & 1.112 & $[0.939,\ 1.308]$ & 1.850 \\
2 & 1.142 & $[0.975,\ 1.315]$ & 1.864 \\
\bottomrule
\end{tabular}
\end{table}

Every interval contains $1.0$ --- no reduction is demonstrated --- and every interval \emph{excludes}
the prediction. Mean ratio $1.06$, seed band $0.21$. \textbf{At $p \approx 0.47$ on this task, the
baseline delivers no detectable variance reduction, and the textbook magnitude is ruled out on 3/3
seeds.} The mean-gradient cosines here are $0.987$--$0.990$, so this is a comparison between
estimators that genuinely do share a mean.
...
\subsection{GRPO starves its own gradient as it converges}
\label{sec:underpowered}

The probe of \cref{sec:eweight} was run at two operating points: the base policy ($p \approx 0.47$)
and after 50 steps of training ($p \approx 0.69$). At the second, \textbf{it stops working} --- and
why it stops is a result about GRPO rather than about the probe.

\begin{table}[H]
\centering
\caption{The same probe at two operating points, three seeds each, $N=160$ batches.}
\label{tab:underpowered}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
& \textbf{$p$} & \textbf{dead groups} & \textbf{$\|\bar{g}\|^2$} & \textbf{NSR} \\
\midrule
base policy      & 0.465 & 0.19 & $594$ & $0.37$--$0.41$ \\
after 50 steps   & 0.694 & 0.46 & $3.1$ & $\mathbf{55}$--$\mathbf{83}$ \\
\bottomrule
\end{tabular}
\end{table}

Across fifty training steps the \emph{signal} collapsed by roughly $600\times$ while the noise fell
only $3\times$: the noise-to-signal ratio rose by more than two orders of magnitude, tracking the
dead-group fraction from $0.19$ to $0.46$. The batch gradient at the later point is overwhelmingly
noise.

This has a sharp consequence for any diagnostic built on comparing gradient \emph{directions}.
$\bar{g}$ is itself an average of $N$ draws, so its own squared relative error is $\mathrm{NSR}/N$,
and two estimates of the \emph{same} vector have expected cosine $\approx 1/(1 + \mathrm{NSR}/N)$.
At $\mathrm{NSR}/N = 0.38$ that ceiling is $0.72$ --- below the $0.90$ agreement floor of
\cref{sec:lennorm}. The floor is unreachable \emph{however perfectly the estimators agree}, so the
check that fires there is vacuous, and reporting it as ``the baselines disagree'' would blame the
estimators for what is a sample-size problem. The probe's verdict at this operating point is
therefore \texttt{underpowered}, not \texttt{rig\_broken}: distinct outcomes, because only the first
is fixed by drawing more batches. Reaching the floor here would need $N \approx 550$--$750$.

\begin{aside}
\textbf{Read this next to \cref{sec:deadintro}.} Dead groups do not merely waste compute --- they
degrade the \emph{quality} of the gradient computed from what remains, and they do so fastest exactly
when the policy is succeeding. The dead-group fraction and the noise-to-signal ratio are two views of
one mechanism, and both get worse as the task is learned. Whether that is why RL runs at this scale
plateau, this phase cannot say; it is the obvious next question.
\end{aside}
...
\begin{itemize}
  ...
  \item \texttt{baseline="global"} --- the baseline is the \emph{full-batch} mean while the cosine
        splits that same batch. Writing $b = (b_A + b_B)/2$, where $b_A, b_B$ are the two \emph{half}-batch mean rewards, each half carries
...
$r_i - \bar{r}_A$ (writing $\bar{r}_A$ for the mean reward over half $A$); under \texttt{global} it is $(r_i - b) - (\bar{r}_A - b) = r_i - \bar{r}_A$. The
...
And the five things measurement changed:
...
\begin{enumerate}
  ...
  \item Once they do, the predicted variance reduction \textbf{does not appear} on 3/3 seeds ---
        consistent with $E_w/E_c \approx 2.9$, a derived and testable number.
  \item As GRPO converges it \textbf{starves its own gradient}: across fifty steps the noise-to-signal
        ratio rose from $\approx 0.4$ to $55$--$83$ as dead groups went $0.19 \to 0.46$.
  \item The half-batch cosine cannot compare baseline arms, and the obvious fix erases the contrast
        rather than the confound.
\end{enumerate}

And three claims that an earlier $n=1$ draft of this document got wrong, all corrected by running two
more seeds:

\begin{enumerate}
  \item Ablation B's leash was reported as having \emph{no} effect on the gap. It has a small one, in
        the \emph{opposite} direction to its purpose (3/3 seeds).
  \item Ablation B's true reward was reported as \emph{falling}. It is flat.
  \item Removing length normalisation was reported as helping \emph{both} arms tested. It reliably
        helps only the arm being Goodharted.
\end{enumerate}

Each was a plausible reading of one seed. None survived three. Every table in
\cref{sec:ablations,sec:surprises} now carries its $n$.
