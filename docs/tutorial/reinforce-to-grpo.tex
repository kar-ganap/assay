...
\texttt{format}   & ablation B & shape only; the answer is discarded & grows: measured $+0.507$ \\
\texttt{tiebreak} & ablation C & $R_{\text{binary}} + 0.001\,|y|$ & exactly $0.001\,|y|$: measured $+0.042$ \\
\bottomrule
\end{tabular}
\end{table}
...
over the final ten steps: gap $= 0.04188$ against $0.001 \times 41.88 = 0.04188$, a ratio of
$1.0000$. When a measurement has a closed form, check it.
...
\begin{table}[H]
\centering
\caption{Ablation B's control arm over training. The proxy is learned in ten steps; the skill never is.}
\label{tab:bgap}
\small
\begin{tabular}{@{}rrrrr@{}}
\toprule
...
20  & 0.992 & 0.297 & $\mathbf{+0.695}$ & 66 \\
...
\textbf{The gap starts negative.} The base model already solves $51.6\%$ of the additions but emits
the required tag only $10.2\%$ of the time, so initially the proxy \emph{understates} true ability.
Reward hacking is not visible at step 0; it has to be trained in.

\textbf{The proxy saturates in ten steps and the skill never moves.} True reward wanders --- $0.516
\to 0.297 \to 0.445$ --- and \emph{ends below where it started}. Optimising the format actively
damaged arithmetic before partially recovering it.
