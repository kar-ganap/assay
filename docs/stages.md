...
| **0.3 · R0** | TinyZero / Countdown. Retires: *"my loop actually learns."* **Scale reopened 2026-08-03 by measurement**: a base-rate screen puts Qwen2.5-1.5B at pass@1 **0.024** with **84.5%** dead groups, and Qwen2.5-3B at **0.059 / 62.0%** on the easiest setting — nothing clears the `<=0.50` band at either scale, so R0 is not well-posed there (a failure would be the task's, not the loop's). `parse_fail` 0.15-0.27 rules out a grader artifact. See `reproductions/README.md` and `phases/phase-0.3-r0-plan.md`. |
