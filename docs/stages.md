...
| **0.3 · R0** | **RETIRED 2026-08-03 on measurement — no longer on the never-cut list.** Three screens, $2.21 of a $10 line: no affordable scale clears the dead-group band (M1: 1.5B **0.845**, 3B **0.620**); the one difficulty axis that preserves the task's search character is **inert** (M3: ~2.5x less arithmetic moved pass@1 by z=0.30); and the cheap route to more compute costs a loop change (M2: sequence ratio ±1σ **[0.218, 1.555]** vs a [0.9, 1.1] band). **The fatal problem was not compute — TinyZero publishes no accuracy, config or metrics, so `reproductions/README.md`'s required Delta has nothing to compute against.** Standing in its place: Phase 0.1 run 7 (0.571→0.923, n=3) plus Phase 0.2's independent-trainer cross-check. **Open limitation, declared:** every task the loop has been shown to learn is pattern completion, **not search**. See `phases/phase-0.3-r0-retro.md`. |
