...
| **0.2 · The ecosystem idiom** | Rebuild the same task as a `verifiers` environment — `load_environment` entrypoint, dataset, `Rubric` with reward functions, `SingleTurnEnv` → `ToolEnv`. Run under `prime-rl` or `trl` `GRPOTrainer` + vLLM. Push to the Hub. Read the `OpenEnv` `reset/step/state` RFC. **Verify the API against source; the docs are thin.** |
