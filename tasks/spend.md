...
| 2026-08-04→05 | 0 · Crawl | 0.4 | **R1 batch 1 — saturation onset.** 9 hosted GRPO runs (3 words × 3 seeds), Llama-3.2-1B-Instruct, 100 steps | Prime Sprints free queue | **17 h 05 m wall clock, measured** (08-04 21:28 → 08-05 14:33), 5-way concurrency | **$0.00 (measured)** | ~$15.50 | **951 M tokens at $0.** All nine reached hack rate 1.0 — R1's reachability gate passes. 4 marked FAILED are post-saturation aborts (10 consecutive zero-advantage batches), not errors. R1's $2 line is untouched. |
| 2026-08-05 | 0 · Crawl | 0.4 | **R1 batch 2 — power on the discriminating pair.** 6 hosted GRPO runs (`ocean`/`midnight` × seeds 3–5), 50 steps, eval interval 3 × 32 examples | Prime Sprints free queue | in flight (5 RUNNING, 1 QUEUED) | **$0.00 (accepted at $0)** | ~$15.50 | Batch 1 gave `ocean` n=3 but `midnight` n=1 uncensored on the pre-registered eval metric. n=6 vs n=6 drops the attainable one-sided p from 0.05 (the n=3 floor) to 0.001. Gradient-side config byte-identical to batch 1; all 15 runs share env version `a5gphk15x62pf7aeadz4qsvm`. |
