...
def test_the_gate_confirms_on_the_predicted_ordering_and_ratio() -> None:
    out = probe_verdict(_stats({"none": 1.0, "global": 0.4, "group_loo": 0.3}), pass_rate=0.6, batches=160)
...
def test_the_gate_calls_a_significant_reversal_falsified() -> None:
    ...
    out = probe_verdict(_stats({"none": 0.14, "global": 1.0, "group_loo": 1.0}), pass_rate=0.43, batches=160)
...
def test_a_reversal_inside_the_noise_is_not_falsified() -> None:
    ...
    """
    out = probe_verdict(
        _stats({"none": 0.88, "global": 1.0, "group_loo": 0.95}, spread=0.10), pass_rate=0.43, batches=160
    )
...
def test_the_gate_reports_partial_between_the_thresholds() -> None:
    assert probe_verdict(_stats({"none": 1.0, "global": 0.62, "group_loo": 0.6}), pass_rate=0.43, batches=160)[
        "verdict"
    ] == "partial"
...
def test_a_wide_interval_is_not_measurable_rather_than_a_null() -> None:
    ...
    """
    out = probe_verdict(_stats({"none": 1.0, "global": 0.9, "group_loo": 0.9}, spread=0.9), pass_rate=0.43, batches=160)
...
def test_disagreeing_mean_gradients_are_a_rig_failure_not_a_result() -> None:
    ...
    """
    out = probe_verdict(_stats({"none": 1.0, "global": 0.3, "group_loo": 0.2}, cosine=0.5), pass_rate=0.43, batches=160)
    assert out["verdict"] == "rig_broken"
    assert "disagree" in out["reason"]


def test_non_finite_statistics_are_a_rig_failure() -> None:
    out = probe_verdict(_stats({"none": float("nan"), "global": 0.3, "group_loo": 0.2}), pass_rate=0.43, batches=160)
    assert out["verdict"] == "rig_broken"
...
def test_the_probe_runs_and_persists_a_verdict(tmp_path: Path) -> None:
    ...
    assert result["verdict"]["verdict"] in {
        "confirmed", "partial", "falsified", "not_measurable", "rig_broken", "underpowered",
    }
...
def test_the_gate_can_fail_upward() -> None:
    ...
    out = probe_verdict(
        _stats({"none": 5.0, "global": 1.0, "group_loo": 0.9}), pass_rate=0.43, batches=160
    )
...
def test_a_correct_result_at_the_base_policy_would_have_failed_the_old_gate() -> None:
    ...
    """
    out = probe_verdict(
        _stats({"none": 1.754, "global": 1.0, "group_loo": 0.95}), pass_rate=0.43, batches=160
    )
    assert out["verdict"] == "confirmed"
...
def test_no_reduction_plus_excluded_prediction_is_a_result_not_an_absence() -> None:
    ...
    """
    out = probe_verdict(
        _stats({"none": 1.02, "global": 1.0, "group_loo": 0.95}, spread=0.10), pass_rate=0.43, batches=160
    )
...
def test_an_interval_spanning_both_is_the_only_uninformative_case(tmp_path: Path) -> None:
    ...
    out = probe_verdict(
        _stats({"none": 1.4, "global": 1.0, "group_loo": 0.95}, spread=0.75), pass_rate=0.43, batches=160
    )
...
def test_a_noise_dominated_mean_is_underpowered_not_a_rig_failure() -> None:
    """The 50-step warmup arm, 2026-08-02: NSR reached 23-61 as ||g_bar||^2 collapsed ~600x.

    ``g_bar`` is itself an average of N noisy draws, so its own squared relative error is ``NSR/N``.
    Two estimates of the *same* vector then have expected cosine ``~1/(1 + NSR/N)`` -- so past
    ``NSR/N = 1/9`` the 0.90 agreement floor is unreachable however perfectly the estimators agree,
    and the check below it is vacuous. Reporting that as "the baselines disagree" blames the
    estimators for what is a sample-size problem.
    """
    out = probe_verdict(
        _stats({"none": 40.0, "global": 60.0, "group_loo": 45.0}, cosine=0.2),
        pass_rate=0.69, batches=160,
    )
    assert out["verdict"] == "underpowered"
    assert "unresolved" in out["reason"]
    assert out["attainable_cosine"]["global"] < 0.90


def test_a_resolved_disagreement_is_still_a_rig_failure() -> None:
    """The length-normalisation case: ``NSR/N`` tiny, so a measured 0.2 is a real disagreement."""
    out = probe_verdict(
        _stats({"none": 1.0, "global": 0.6, "group_loo": 0.5}, cosine=0.2),
        pass_rate=0.47, batches=160,
    )
    assert out["verdict"] == "rig_broken"
    assert "resolved" in out["reason"]


def test_the_resolution_check_runs_before_the_agreement_check() -> None:
    """Ordering is the point. An unresolved mean cannot be trusted even when it happens to agree."""
    out = probe_verdict(
        _stats({"none": 40.0, "global": 60.0, "group_loo": 45.0}, cosine=0.99),
        pass_rate=0.69, batches=160,
    )
    assert out["verdict"] == "underpowered"
